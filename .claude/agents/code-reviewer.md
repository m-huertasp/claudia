...
1. **Determine target** — Use the following priority order to find what to review:
   ...
   - If a PR number/URL is provided, delegate to the `/pr-review` command.
...
When invoked via `/python-review` or `/nextflow-review`, **that command's checklist and phases govern the review entirely**. The categories below are used only for standalone invocations (no active command).
...
When invoked via a specific command (e.g., `/pr-review`, `/python-review`), follow that command's artifact naming and structure exactly. For standalone use, save the report as `code-review_report_DDMMYY.md` in the current directory.
...
When available, also check project-specific conventions from `CLAUDE.md` or contributing guidelines:
