...
3. **Explore the codebase.** Dispatch the `researcher` agent for a read-only survey: entry points, module layout, data flow, dependencies, test conventions. For larger or layered codebases, also dispatch `code-explorer` for an architecture trace. Honour the `$ARGUMENTS` focus hint if given.
