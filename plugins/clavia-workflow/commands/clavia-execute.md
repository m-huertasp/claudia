...
5. **When all tasks are done**, update `STATE.md`: next step = `/clavia-verify`.

## Review gate

Routine code edits inside a task do **not** need a per-change gate — they are
reviewed at `/clavia-verify`. But anything outward-facing (a push, a PR) is
never done here — that is `/clavia-ship`. If an executor needs a decision,
